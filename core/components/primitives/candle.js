...
export default class CandleExt {
    ...
    draw(data) {
        ...
        if (data.w > 1.5) {
            ...
            let s = green ? 1 : -1
            const bx = Math.floor(data.x - hw - 1)
            const bw = Math.floor(hw * 2 + 1)
            const bh = s * Math.max(h, max_h)
            this.ctx.fillRect(bx, data.c, bw, bh)

            // border around the body -> hollow / outlined candles (colorCandleBorderUp/Dw,
            // falls back to the wick color). This is the patch for app-style bordered candles.
            const border_color = green
                ? (this.style.colorCandleBorderUp || wick_color)
                : (this.style.colorCandleBorderDw || wick_color)
            if (border_color) {
                this.ctx.lineWidth = 1
                this.ctx.strokeStyle = border_color
                this.ctx.strokeRect(bx + 0.5, data.c, bw - 1, bh)
            }
