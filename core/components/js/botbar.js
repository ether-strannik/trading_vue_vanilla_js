...
export default class Botbar {
    ...
    format_date(p) {
        ...
        // app-supplied axis tick formatter (LWC tickMarkFormatter(seconds, TickMarkType))
        const tmf = this.$p.tickMarkFormatter
        if (tmf) {
            let type
            if (p[2] === YEAR || Utils.year_start(t) === t) type = 0
            else if (p[2] === MONTH || Utils.month_start(t) === t) type = 1
            else if (Utils.day_start(t) === t) type = 2
            else type = 3
            return tmf(t / 1000, type)
        }
    ...
    format_cursor_x() {
        ...
        t = this.grid_0.ti_map.i2t(t)
        // app-supplied crosshair time formatter (LWC localization.timeFormatter(seconds))
        const tf = this.$p.timeFormatter
        if (tf) return tf(t / 1000)
