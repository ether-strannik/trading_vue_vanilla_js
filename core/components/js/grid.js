...
// (hammerjs + hamsterjs stripped — zero-dep; native input is wired in the engine shell)
...
export default class Grid {
    ...
    // Hammer/Hamster gesture wiring stripped for zero-dep. The engine shell wires native
    // pointer/wheel events and feeds range/cursor; the interaction methods below (mousedrag,
    // mousezoom, emit_cursor_coord, ...) stay as the author's logic, to be driven natively.
    listeners() { /* no-op (native input lives in the engine) */ }
    ...
    grid() {
        ...
        // vertical lines
        this.ctx.strokeStyle = this.$p.colors.gridVert || this.$p.colors.grid
        ...
        // horizontal lines
        this.ctx.strokeStyle = this.$p.colors.gridHorz || this.$p.colors.grid
